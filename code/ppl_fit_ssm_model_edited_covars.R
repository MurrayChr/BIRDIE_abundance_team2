...
#' @param model_file A path to the JAGS model file
...
#'
#' @examples
ppl_fit_ssm_model_edited_covars <- function(counts, model_file, ...){

  ### HERE WE CONFIGURE COVARIATES ###
    ...
    covts_u <- counts %>%
        ...
        dplyr::select(site_id, year_id, TMAXX, RAINFALL, WIND) %>%
        dplyr::mutate(dplyr::across(.cols = c(TMAXX, RAINFALL, WIND), .fns = ~scale(.x)))
    ...
    inits <- function(){
        list(mu_ini = runif(data$nsites, 0, 10),
             ...
             # G = matrix(rnorm(data$M * data$nsites, 0, 1), ncol = data$M),  # these are initial values for reg coefficients
             # mu_g = rnorm(data$M, 0, 0.5),  # these are initial values for random effect means
             # sig_g = rexp(data$M, 1),       # these are sds for random effects
